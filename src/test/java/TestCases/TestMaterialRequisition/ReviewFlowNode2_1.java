...
public class ReviewFlowNode2_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("849340730873741312");
outputtextList.get(dr).setTestCaseDesc("");
outputtextList.get(dr).setSkipOtherTCsWhenFailed(Integer.parseInt(skipOtherTCsWhenFailed));
}
@Test(timeOut = 180000,description="waitForElementToBeClickable:等待审核通过按钮可点击",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
public void Step0(String dr,String paramGroup) throws Exception {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
String testObject="带颜色的审核按钮";
String byString= pageObject4.POString.get(testObject);
String testData ="";
outputtextList.get(dr).setMethodName("waitForElementToBeClickable：等待审核通过按钮可点击");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.waitForElementToBeClickable(dr,byString,testData);
}
@Test(dependsOnMethods ="Step0",timeOut = 180000,description="waitingForClickOrInput:点击审核通过",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step2",timeOut = 180000,description="waitingForClickOrInput:填写审核意见",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
outputtextList.get(dr).setMethodName("waitingForClickOrInput：填写审核意见");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.waitingForClickOrInput(dr,byString,testData);
}
@Test(dependsOnMethods ="Step3",timeOut = 180000,description="KeyboardOperation:移出光标",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
String byString= pageObject4.POString.get(testObject);
String testData ="Tab";
outputtextList.get(dr).setMethodName("KeyboardOperation：移出光标");
outputtextList.get(dr).setScreenshot(true);
...
@Test(dependsOnMethods ="Step5",timeOut = 180000,description="waitForAttributeContains:等待加载提示消失",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
