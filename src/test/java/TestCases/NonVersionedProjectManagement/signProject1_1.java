...
public class signProject1_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("893130531498098688");
outputtextList.get(dr).setTestCaseDesc("会签项目_角色1");
...
@Test(dependsOnMethods ="Step9",timeOut = 180000,description="MoveToElement:点击项目管理菜单",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step10",timeOut = 180000,description="waitingForClickOrInput:点击立项管理菜单",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step11",timeOut = 180000,description="filterByTitle:输入项目名称查询",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step13",timeOut = 180000,description="Verify_AnyContain:校验文本",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step14",timeOut = 180000,description="clickByTitle:进入项目详情",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step15",timeOut = 180000,description="waitingForClickOrInput:会签通过",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
public void Step16(String dr,String paramGroup) throws Exception {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
String testObject="会签通过按钮";
String byString= pageObject6.POString.get(testObject);
String testData ="";
...
@Test(dependsOnMethods ="Step16",timeOut = 180000,description="waitingForClickOrInput:会签通过确定",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
String byString= pageObject6.POString.get(testObject);
String testData ="";
outputtextList.get(dr).setMethodName("waitingForClickOrInput：会签通过确定");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.waitingForClickOrInput(dr,byString,testData);
}
