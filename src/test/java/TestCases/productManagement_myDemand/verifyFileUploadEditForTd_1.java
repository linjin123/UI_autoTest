...
public class verifyFileUploadEditForTd_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("887658247337541632");
outputtextList.get(dr).setTestCaseDesc("需求管理_技术文档保存成功校验");
...
@Test(dependsOnMethods ="Step1",timeOut = 180000,description="CurrentWindowClose:关闭页面",alwaysRun = true)
@Parameters({"driverID","paramGroup"})
