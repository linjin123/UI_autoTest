...
public class signProduction_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("887658257689083904");
...
@Test(dependsOnMethods ="Step4",timeOut = 180000,description="waitForAttributeContains:等待加载",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step8",timeOut = 180000,description="filterByTitle:查询产品标题",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
outputtextList.get(dr).setMethodName("filterByTitle：查询产品标题");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.filterByTitle(dr,byString,testData);
}
@Test(dependsOnMethods ="Step11",timeOut = 180000,description="waitingForClickOrInput:进入产品详情",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step12",timeOut = 180000,description="waitingForClickOrInput:会签通过",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step13",timeOut = 180000,description="waitingForClickOrInput:会签意见",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step14",timeOut = 180000,description="waitingForClickOrInput:会签确定",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
