...
public class addDemandNotSelectMoudle_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("887658242283405312");
outputtextList.get(dr).setTestCaseDesc("需求管理_未选择模块新建需求");
...
@Test(dependsOnMethods ="Step0",timeOut = 180000,description="MoveToElement:点击管理员",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step7",timeOut = 180000,description="waitForObjectAppear:等待元素可见",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
String byString= pageObject22.POString.get(testObject);
String testData ="";
outputtextList.get(dr).setMethodName("waitForObjectAppear：等待元素可见");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.waitForObjectAppear(dr,byString,testData);
}
...
@Test(dependsOnMethods ="Step9",timeOut = 180000,description="waitingForClickOrInput:点击需求池",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step10",timeOut = 180000,description="waitingForClickOrInput:点击新建需求按钮",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step11",timeOut = 180000,description="waitingForClickOrInput:所属产品选择",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step12",timeOut = 180000,description="waitingForClickOrInput:所属产品选择",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step13",timeOut = 180000,description="waitingForClickOrInput:所属产品选择",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
