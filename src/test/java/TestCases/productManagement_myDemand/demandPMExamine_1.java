...
public class demandPMExamine_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("887658241226440704");
outputtextList.get(dr).setTestCaseDesc("需求管理_产品经理审批");
...
@Test(dependsOnMethods ="Step12",timeOut = 180000,description="waitingForClickOrInput:点击标题进入",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
String testData ="";
outputtextList.get(dr).setMethodName("waitingForClickOrInput：点击标题进入");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.waitingForClickOrInput(dr,byString,testData);
}
@Test(dependsOnMethods ="Step13",timeOut = 180000,description="waitingForClickOrInput:点击审批",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step14",timeOut = 180000,description="waitingForClickOrInput:审批意见输入",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
@Test(dependsOnMethods ="Step15",timeOut = 180000,description="waitingForClickOrInput:点击审批确定",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
