...
public class ChooseStructuralDevelopmentEngineer_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("849340760653299712");
outputtextList.get(dr).setTestCaseDesc("");
outputtextList.get(dr).setSkipOtherTCsWhenFailed(Integer.parseInt(skipOtherTCsWhenFailed));
}
@Test(timeOut = 180000,description="ScrollIntoView:滚动至结构开发工程师",alwaysRun = false)
...
String testData ="";
outputtextList.get(dr).setMethodName("ScrollIntoView：滚动至结构开发工程师");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.ScrollIntoView(dr,byString,testData);
}
@Test(dependsOnMethods ="Step0",timeOut = 180000,description="MouseAction:结构开发工程师",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
outputtextList.get(dr).setMethodName("MouseAction：结构开发工程师");
outputtextList.get(dr).setScreenshot(true);
...
@Test(dependsOnMethods ="Step4",timeOut = 180000,description="ScrollIntoView:滚动至确认",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
String byString= pageObject9.POString.get(testObject);
String testData ="";
outputtextList.get(dr).setMethodName("ScrollIntoView：滚动至确认");
outputtextList.get(dr).setScreenshot(true);
ApplicationActions.ScrollIntoView(dr,byString,testData);
}
@Test(dependsOnMethods ="Step5",timeOut = 180000,description="waitingForClickOrInput:点击确认",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
