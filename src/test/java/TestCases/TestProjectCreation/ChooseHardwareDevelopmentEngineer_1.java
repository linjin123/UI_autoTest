...
public class ChooseHardwareDevelopmentEngineer_1 extends TestSetup{
private String TCParam="";
@BeforeTest(alwaysRun = true)
@Parameters({"driverID","skipOtherTCsWhenFailed"})
public void beforeTest(String dr,String skipOtherTCsWhenFailed) {
dr= Reporter.getCurrentTestResult().getTestContext().getSuite().getName();
outputtextList.get(dr).setTestCaseID("849340765850042368");
outputtextList.get(dr).setTestCaseDesc("");
outputtextList.get(dr).setSkipOtherTCsWhenFailed(Integer.parseInt(skipOtherTCsWhenFailed));
}
@Test(timeOut = 180000,description="MouseAction:硬件开发工程师",alwaysRun = false)
...
outputtextList.get(dr).setMethodName("MouseAction：硬件开发工程师");
...
@Test(dependsOnMethods ="Step5",timeOut = 180000,description="ScrollIntoView:滚动",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
...
String byString= pageObject9.POString.get(testObject);
String testData ="";
outputtextList.get(dr).setMethodName("ScrollIntoView：滚动");
outputtextList.get(dr).setScreenshot(true);
...
@Test(dependsOnMethods ="Step7",timeOut = 180000,description="waitForAttributeContains:等待加载",alwaysRun = false)
@Parameters({"driverID","paramGroup"})
